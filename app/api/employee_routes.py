...
def employee_dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    entries = db.query(TimeEntry).filter(User.username == user["username"], TimeEntry.user_id == User.id).order_by(TimeEntry.check_in.desc()).all()
    return templates.TemplateResponse("employee.html", {"request": request, "user": user, "entries": entries})
